...
def format_averages_block(algorithms):
    means_filtered = [
        np.round(get_mean(MEANS[algorithm], exclude="ant"), 1)
        for algorithm in algorithms
    ]
    means_all = [
        np.round(get_mean(MEANS[algorithm], exclude=None), 1)
        for algorithm in algorithms
    ]

    means_all = [
        means if "ant-medium-expert-v2" in MEANS[algorithm] else "$-$"
...
def format_table(algorithms, buffers, environments):
    ...
    blocks = "".join(
        format_buffer_block(algorithms, buffer, environments) for buffer in buffers
    )
    table = f"{table_prefix}" f"{blocks}" f"{table_suffix}"
    return table
